...
const Dropdown = (props) => {
    ...
    const { register, unregister, name, setValue, watch } = props
    ...
    const defaultValue = watch(name, "get")
      ...
      <Container relative={!props.borderNone} ref={bind.ref} style={{...hide}}>
        <Wrapper isOpen={isOpen}>
          ...
          <SelectList isOpen={isOpen}>
            {(props.options || []).map((e) => (
              <li key={e.value} style={{fontWeight: 500, color: e.color ? e.color : "currentColor"}} onClick={() => setValue(name, e.value)}>{e.label}</li>
            ))}
          </SelectList>
        </Wrapper>
      </Container>
